...
def merge_dynamic_static(dynamic_path,static_path,merge_path):

    output_path = merge_path
    if not os.path.exists(output_path):
        os.makedirs(output_path)

    dynamic_path = os.path.join(dynamic_path, 'test_vis')
    static_path = os.path.join(static_path, 'test_vis')
    ...
    for figure in dynamic_figures:
        ...
        cv2.imwrite(os.path.join(output_path, figure), static_fig)
